@icon("res://BehaviorTree/icons/btconsider.svg")
extends BTUtilityScorer
class_name BTConsideration
...
# Main update method that calculates the consideration value. Will be called by its parent BTScorer node or BTLeaf node.
func _update_score(blackboard):
	consider_data = gather_consider_data(blackboard)
	var value: float = _apply_response_curve(consider_data) if not use_custom_response_function else apply_response_function(consider_data)
	set_bt_score(value)
...
func gather_consider_data(blackboard: BTBlackboard): return 0.0
